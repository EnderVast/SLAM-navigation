...
def spin():
    ...
    for i in range(n):
        lv_rot, rv_rot = ppi.set_velocity(command, turning_tick=wheel_vel, time=turn_time)
        drive_meas_rot = operate.control(lv_rot, rv_rot, turn_time)
        operate.update_slam(drive_meas_rot)
        time.sleep(0.2)
...
if __name__ == "__main__":
    ...
    for i in range(len(aruco_true_pos)):
        ...
        x = aruco_true_pos[i][0]*10
        y = aruco_true_pos[i][1]*10
        # ox.append(x)
        # oy.append(y)

        # Draw box for each obstacle
        for i in range(-1, 2, 1):  # top line
            for j in range(-1, 2, 1):  # top line
                ox.append(x + i)
                oy.append(y + j)

    # Append fruit as obstacle
    for i in range(len(fruits_true_pos)):
            #all_obstacles.append(Circle(aruco_true_pos[i][0], aruco_true_pos[i][1], 0.2))
            x = fruits_true_pos[i][0]*10
            y = fruits_true_pos[i][1]*10
            ox.append(x)
            oy.append(y)
    ...
    for i in range(len(fruits_true_pos)):
        ...
        # Simulate 
        resolution = 0.5
        simulate_astar(ox, oy, start[0], start[1], goal[0], goal[1], baseline*10*1.2, resolution)
        ...
        a_star = AStarPlanner(ox=ox, oy=oy, resolution=resolution, rr=baseline*10*1.2)
