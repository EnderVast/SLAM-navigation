...
class Operate:
    def __init__(self, args, ppi_operate):
    ...
    def initAruco(self, position):
        measurements = []
        for i in range(len(position)):
            arucoMarker = np.zeros((2,1))
            arucoMarker[0][0] = position[i][0]
            arucoMarker[1][0] = position[i][1]
            lm_measurement = measure.Marker(arucoMarker, i+1)
            #print(lm_measurement.tag)
            #lm_measurement.tag = i
...
if True:
    ...
    # parser.add_argument("--ip", metavar='', type=str, default='192.168.137.254')
    ...
    # while start:
    #     operate.update_keyboard()   # Setting correct [command] based on keyboard
    #     operate.take_pic()



    #     drive_meas = operate.control()  # Driving the robot and returning lv, rv and dt as drive_meas
    #     operate.update_slam(drive_meas) # Update SLAM
