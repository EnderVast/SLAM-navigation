...
# 0.045 full battery
# 0.05 med battery
actual_turn_time_bias_spin = 0.042
...
def spin(n, m):
    ...
    for j in range(m):
        for i in range(n):
            ...
            #print(turn_time)
            print("Turning for {:.2f} seconds".format(turn_time))
            lv_rot, rv_rot = ppi.set_velocity(command, turning_tick=wheel_vel, time=turn_time + actual_turn_time_bias_spin)
            ppi.set_velocity([0, 0])
            # Rotation EKF
            drive_meas_rot = operate.control(lv_rot, rv_rot, turn_time + EKF_turn_time_bias)
            operate.update_slam(drive_meas_rot)
...
def getClosestGoal(sx, sy, fx, fy):
    coordinates = []
    distance = []
    for i in range(-1, 2, 1):
        for j in range(-1, 2, 1):
            ...
            distance.append(math.sqrt((x-sx)**2 + (y-sy)**2))
    ...
    gx = coordinates[index][0]
    gy = coordinates[index][1]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--map", type=str, default='monday_3_fruit.txt')
    ...
    for i in range(len(fruits_true_pos)):
        ox.append(fruits_true_pos[i][0]*10)
        oy.append(fruits_true_pos[i][1]*10)
    ...
    while 1:
        for i in range(len(fruits_true_pos)):
            ...
            goal = np.array(fruits_true_pos[i])
            goal = getClosestGoal(start[0], start[1], goal[0], goal[1])
            ...
            for k in range(len(rx)):
                waypoints.append([rx[k]/10, ry[k]/10])
