...
class RRTC:
    ...
    def planning(self):
        ...
        #TODO: Complete the planning method ----------------------------------------------------------------
            ...
            index = self.get_nearest_node_index(self.end_node_list, new_node)
            distance, _ = self.calc_distance_and_angle(new_node, self.end_node_list[index])
            ...
            if distance <= self.expand_dis:
                if self.is_collision_free(self.end_node_list[index]):
                    self.end_node_list.append(self.end_node_list[index])
                    return self.generate_final_course(len(self.start_node_list) - 1, len(self.end_node_list) - 1)
